(function initRoadGraph(){
  ...
  (function injectApronBridges(){
    ...
    // kind: nsBridge=T2↔T3 only; t3Local; northSvc=V↔J; hbs=T1 E–W Hauptbetriebsstraße
      ...
      // A20–A40 stand-face service (white Betriebswege SW of HBS, west of mid taxi)
      {kind:'cargoW', pts:[[50.04680,8.56420],[50.04640,8.56500],[50.04600,8.56550],[50.04550,8.56570],[50.04500,8.56530],[50.04450,8.56450],[50.04420,8.56390],[50.04390,8.56320],[50.04370,8.56220]]},
      {kind:'cargoW', pts:[[50.04675,8.56340],[50.04620,8.56420],[50.04560,8.56480],[50.04500,8.56460],[50.04440,8.56400],[50.04400,8.56340],[50.04375,8.56240]]},
      // Gebäude 125 / Betriebszentrale Ost service — link V apron to HBS
      {kind:'vfOst', pts:[[50.04890,8.59520],[50.04850,8.59440],[50.04800,8.59320],[50.04750,8.59200],[50.04720,8.59100],[50.04685,8.59000]]},
      {kind:'vfOst', pts:[[50.04890,8.59460],[50.04840,8.59300],[50.04780,8.59120],[50.04730,8.58950],[50.04690,8.58750]]}
    ];
    ...
    (function buildGseSkin(){
      const nodes = window._roadNodes;
      const setsAll = [window._synthNodes, window._cargoWNodes, window._hbsNodes,
        window._vfOstNodes, window._nsBridgeNodes, window._northSvcNodes];
      // Strict skin = only painted gray Betriebswege (HBS / vfOst / cargoW). NS excluded
      // so mid-field taxi OSM edges cannot launder as "GSE".
      const setsStrict = [window._cargoWNodes, window._hbsNodes, window._vfOstNodes];
      const mk = (sets, radiusCells) => {
        const cells = new Set();
        const addAround = (la,ln) => {
          const ia = Math.round(la*4500), ib = Math.round(ln*4500); // ~25 m
          for (let di=-radiusCells; di<=radiusCells; di++)
            for (let dj=-radiusCells; dj<=radiusCells; dj++)
              cells.add((ia+di)+','+(ib+dj));
        };
      ...
      const cellsAll = mk(setsAll, 1);
      const cellsStrict = mk(setsStrict, 1);
      window._gseCells = cellsAll;
      window._gseCellsStrict = cellsStrict;
      window._onGseSkin = function(lat, lng){
        return cellsAll.has(Math.round(lat*4500)+','+Math.round(lng*4500));
      };
      window._onGseSkinStrict = function(lat, lng){
        return cellsStrict.has(Math.round(lat*4500)+','+Math.round(lng*4500));
      };
    })();
  ...
  window._isRollfeld = function(lat, lng){
    // --- GSE carve-outs FIRST (tight tubes only — do NOT trust wide GSE skin here) ---
    ...
    // West drop HBS→cargo / A20–A40 face: ONLY thin building-side lane (not midfield)
    if (lng >= 8.5605 && lng <= 8.5670 && lat >= 50.0420 && lat <= 50.0471) return false;
    // Thin NS GSE bridges T2↔T3 — ~70 m tube (path densify stays inside; OSM taxi still outside)
    if (lat >= 50.0315 && lat <= 50.0482){
      const t = Math.max(0, Math.min(1, (50.0472 - lat) / 0.0155));
      if (Math.abs(lng - (8.5910 - t*0.0080)) < 0.00100) return false;
      if (Math.abs(lng - (8.5828 - t*0.0038)) < 0.00100) return false;
      if (Math.abs(lng - (8.5748 + t*0.0044)) < 0.00100) return false;
    }
    ...
    // Vorfeld Ost stand-approach (V/W aisles) + Gebäude 125 service
    if (lat >= 50.0476 && lat <= 50.0502 && lng >= 8.579 && lng <= 8.5965) return false;
    // Synthetic spine cell only on OWNED kinds (HBS/cargo/vfOst) — NOT loose NS mid belt
    if (window._onGseSkinStrict && window._onGseSkinStrict(lat, lng)) return false;
  ...
  // Preferred open Vorfeld / outer service ring (yellow-line style)
    ...
    // MAIN T1/T2 apron E–W HBS south of A–E piers (gray/white Betriebswege only)
    if (lng > 8.552 && lng < 8.596 && lat >= 50.04615 && lat < 50.04795) return true;
    // Vorfeld Ost service lanes + Gebäude 125 apron
    if (lng >= 8.579 && lng <= 8.5965 && lat >= 50.04695 && lat <= 50.0506) return true;
    // Thin NS authorized spines only (not whole midfield)
    if (lat >= 50.0315 && lat <= 50.0482){
      const t = Math.max(0, Math.min(1, (50.0472 - lat) / 0.0155));
      if (Math.abs(lng - (8.5910 - t*0.0080)) < 0.00100) return true;
      if (Math.abs(lng - (8.5828 - t*0.0038)) < 0.00100) return true;
      if (Math.abs(lng - (8.5748 + t*0.0044)) < 0.00100) return true;
    }
    // Cargo F GSE pads + truck lanes (not taxi)
    if (lng > 8.532 && lng <= 8.552 && lat >= 50.0388 && lat <= 50.0438) return true;
    // West drop / A20–A40 stand face service
    if (lng >= 8.5605 && lng <= 8.5675 && lat >= 50.0425 && lat <= 50.0471) return true;
    // Cargo E–W service corridor (north of rollfeld)
    if (lng > 8.548 && lng < 8.5658 && lat >= 50.0405 && lat <= 50.0448) return true;
    // T3 OPEN apron south of G/H/J piers (service)
  ...
  while(heap.length){
    ...
    for (let k=0;k<nbrs.length;k++){
      ...
      if (apronPrefer){
        ...
        const westDrop = mlng >= 8.5605 && mlng <= 8.5672 && mlat >= 50.0420 && mlat <= 50.04715;
        const onHbs = window._hbsNodes && (window._hbsNodes.has(u) || window._hbsNodes.has(v));
        const onVf = window._vfOstNodes && (window._vfOstNodes.has(u) || window._vfOstNodes.has(v));
        const onStrict = window._onGseSkinStrict && (window._onGseSkinStrict(mlat, mlng) || window._onGseSkinStrict(na[0], na[1]) || window._onGseSkinStrict(nb[0], nb[1]));
        ...
        // HARD ban taxi/rollfeld. Allow only real gray Betriebswege (strict spines).
        if (isRoll(mlat, mlng) || isRoll(na[0], na[1]) || isRoll(nb[0], nb[1])){
          let allow = false;
          if (opts.destCargo) allow = !!(onCargo || westDrop || onHbs || (onStrict && mlng <= 8.5675));
          else if (opts.needNS || opts.destT3) allow = !!(onNS || onHbs || onVf || onStrict);
          else allow = !!(onCargo || westDrop || onHbs || onVf || onStrict);
          // Never launder mid-taxi OSM via fat NS GSE skin on local T1/T2 hops
          if (!(opts.needNS || opts.destT3) && onNS && mlat < 50.0462) allow = false;
          if (!allow) continue;
          w = w * 1.25 + 14;
        }
        // HARD ban residual mid-apron green/gray open band (OSM L/N taxi OR empty apron)
        // User: only WHITE service roads — anything midfield east of west-drop is illegal for T1/T2.
        if (mlat >= 50.0425 && mlat < 50.04640 && mlng >= 8.548 && mlng <= 8.596){
          let allow = false;
          if (opts.destCargo) allow = !!(onCargo || westDrop || (onStrict && mlng <= 8.5675));
          else if (opts.needNS || opts.destT3) allow = !!(onNS || onStrict);
          else allow = !!(onCargo || westDrop || (onStrict && mlng <= 8.5675));
          if (!(opts.needNS || opts.destT3 || opts.destCargo)){
            if (!(westDrop || onCargo)) allow = false;
            if (mlng > 8.5678) allow = false;
          }
          if (!allow) continue;
        }
        // Ban east outer green / Ellis Road hillside detours (cyan loop east of Geb.125)
        if (mlng > 8.5952 && mlat < 50.0495 && !(onVf || onHbs || onStrict)){
          w = w * 25 + 400;
          if (mlat < 50.0478) continue;
        }
        // Strong HBS preference for any E–W hop above V/HBS band
        if (onHbs || (mlat >= 50.04635 && mlat <= 50.04805 && mlng >= 8.555 && mlng <= 8.5955)){
          w *= 0.48;
        }
        if (onVf) w *= 0.62;
        ...
        // A20–A40 / west stand face needs the south drop; pure HBS A/B/C/D/E northern gates must not
        const destWestFace = !!(destLL && destLL[0] < 50.04635 && destLL[1] >= 8.560 && destLL[1] <= 8.5685);
        const startWestFace = !!(startLL && startLL[0] < 50.04635 && startLL[1] >= 8.560 && startLL[1] <= 8.5685);
        if (t1Local && mlat < 50.04615){
          if (!(westDrop && (destWestFace || startWestFace || opts.destCargo))){
            continue; // hard: no mid/green for V↔A4 style north pier trips
          }
        }
        if (t1Local && mlat < 50.0420 && !(opts.destCargo || destWestFace || startWestFace)){
          w = w * 6.5 + 180;
        }
...
function computeRoute(fromLatLng, toLatLng, destMeta){
  ...
  // Gebäude 125 / Vorfeld Ost → west piers: force onto HBS white service, never Ellis/green east loop
  if (fromLatLng[1] >= 8.585 && toLatLng[1] <= 8.580 && fromLatLng[0] > 50.044){
    const altH = projectOntoRoad(50.04685, Math.min(fromLatLng[1], 8.5935), true, true, true);
    if (altH && altH.dist < 480 && !isRollf(altH.point[0], altH.point[1])){
      fromSnap = Object.assign({}, altH, { _leaveRoll:true, _gps:fromLatLng });
    }
  }
  // A20–A40 face: snap dest onto west-drop / A-face service (not taxi mid)
  if (/^A(2[6-9]|3[0-9]|40)/i.test(destId) || (toLatLng[0] < 50.0460 && toLatLng[0] > 50.0430 && toLatLng[1] >= 8.560 && toLatLng[1] <= 8.5685)){
    const altA = projectOntoRoad(Math.min(toLatLng[0] + 0.0010, 50.0466), Math.min(toLatLng[1] + 0.0004, 8.5655), true, true, false);
    if (altA && altA.dist < 280 && !isRollf(altA.point[0], altA.point[1])) toSnap = altA;
  }
  ...
  if (!bestRes){
    // Last resort: hug HBS band then dest approach (never chord across taxi green)
    const hbsFrom = projectOntoRoad(50.04675, fromLatLng[1], true, true, true);
    const hbsTo = projectOntoRoad(50.04675, toLatLng[1], true, true, false);
    if (hbsFrom && hbsTo){
      const s2 = [hbsFrom.a, hbsFrom.b].filter((v,i,a)=>a.indexOf(v)===i);
      const e2 = [hbsTo.a, hbsTo.b].filter((v,i,a)=>a.indexOf(v)===i);
      let br=null, be=Infinity;
      for (const s of s2) for (const t of e2){
        const res = astarRoad(s, t, Object.assign({}, optsBase, { apronPrefer:true, banTunnel:true }));
        if (res && res.dist < be){ be=res.dist; br=res; }
      }
      if (br){
        bestRes = br;
        fromSnap = Object.assign({}, hbsFrom, { _leaveRoll:true, _gps:fromLatLng });
        toSnap = hbsTo;
      }
    }
  }
  if (!bestRes){
    // Still no graph path: follow densified HBS corridor points (no green diagonal)
    const via = [];
    const aLng = fromLatLng[1], bLng = toLatLng[1];
    const steps = Math.max(6, Math.ceil(Math.abs(bLng - aLng) / 0.0015));
    via.push([Math.max(fromLatLng[0], 50.0466), aLng]);
    for (let i=1;i<steps;i++){
      const t=i/steps;
      via.push([50.04675, aLng + (bLng-aLng)*t]);
    }
    via.push([Math.max(toLatLng[0], 50.0435) < 50.0460 ? Math.min(toLatLng[0]+0.0003, 50.0462) : toLatLng[0], bLng]);
    via.push(toLatLng);
    return {
      latlngs: densifyLatLngs(via, 18),
      dist: via.reduce((s,_,i)=>i?s+haversine(via[i-1],via[i]):0,0),
      viaRoads:false,
      startSnap: via[0],
      endSnap: via[via.length-1],
      _snapKey: 'hbs-fallback'
    };
  }
...
const POSITIONS = [
  ...
  {id:"GEB-125", lat:50.04885, lng:8.59470, cat:"landmark", desc:"Gebäude 125 · Betriebszentrale Ost"},
...
/** Fixed Demo start — Gebäude 125 Betriebszentrale Ost. */
const DEMO_ORIGIN = Object.freeze({ lat: 50.04885, lng: 8.59470, heading: 250, id: 'GEB-125' });
...
function compileRoute(){
  ...
  let from;
  if (typeof demoMode !== 'undefined' && demoMode && typeof DEMO_ORIGIN !== 'undefined'){
    const cur = [currentPosition.lat, currentPosition.lng];
    const home = [DEMO_ORIGIN.lat, DEMO_ORIGIN.lng];
    const leftHome = (typeof haversine === 'function') ? haversine(cur, home) > 90 : false;
    const onActiveRoad = !!(lastRoute && lastRoute.latlngs && lastRoute.latlngs.length > 2 && leftHome);
    from = onActiveRoad ? cur : home;
  } else {
    from = [currentPosition.lat, currentPosition.lng];
  }
  let totalDist = 0;
  let totalGeo = 0;
  const legs = [];
...
function setCurrentPosition(lat,lng,opts={}){
  ...
  // Demo hard-lock: only demo ticks may move the vehicle (block GPS/manual/untagged)
  if (typeof demoMode !== 'undefined' && demoMode && opts.source !== 'demo' && opts.source !== 'demo-tick'){
    return;
  }
  currentPosition = {
    lat, lng,
...
function startDemo(reason){
  ...
  try{ if(gpsWatchId!=null){ navigator.geolocation.clearWatch(gpsWatchId); gpsWatchId=null; } }catch(e){}
  let hdg = DEMO_ORIGIN.heading;
  let lat = DEMO_ORIGIN.lat, lng = DEMO_ORIGIN.lng;
  let demoPathIdx = 0;
  if(demoTimer) clearInterval(demoTimer);
  _manualPosition = { lat, lng };
  lastRoute = null;
  if(routeLayer && routeLayer.clearLayers) try{ routeLayer.clearLayers(); }catch(e){}
  setCurrentPosition(lat, lng, { accuracy:8, heading:hdg, speed: 0, source:'demo' });
  if(selectedTarget){
    drawRoute();
    if(lastRoute && lastRoute.latlngs && lastRoute.latlngs.length > 2){
      lat = lastRoute.latlngs[0][0]; lng = lastRoute.latlngs[0][1];
      demoPathIdx = 0;
      setCurrentPosition(lat, lng, { accuracy:8, heading:hdg, speed: 0, source:'demo' });
    }
  }
  followMode = true; _userMovedMap = false; updateFollowFab(); followCamera(true);
  try{ map.setView([DEMO_ORIGIN.lat, DEMO_ORIGIN.lng], Math.max(map.getZoom(), 16), {animate:true}); }catch(e){}
  showToast((t('gps_demo')||'Demo') + ' · Gebäude 125' + (reason ? ' — '+reason : ''), 'warn');
  updateDemoButton();
  demoTimer = setInterval(()=>{
    if(!demoMode) return;
    // Drive along active route centerline when navigating; else idle at Geb.125
    const path = (lastRoute && lastRoute.latlngs && lastRoute.latlngs.length > 2) ? lastRoute.latlngs : null;
    if (path){
      const sp = 28/3.6; // m/s along service roads
      // advance ~sp meters each tick
      let remain = sp;
      while(remain > 0.5 && demoPathIdx < path.length-1){
        const a = path[demoPathIdx], b = path[demoPathIdx+1];
        const seg = haversine(a,b) || 0.01;
        if (remain >= seg){
          remain -= seg;
          demoPathIdx++;
          lat = b[0]; lng = b[1];
      ...
      if (demoPathIdx >= path.length-1){
        lat = path[path.length-1][0]; lng = path[path.length-1][1];
        setCurrentPosition(lat, lng, { accuracy:6, heading:hdg, speed: 0, source:'demo-tick' });
      } else {
        setCurrentPosition(lat, lng, { accuracy:6, heading:hdg, speed: sp, source:'demo-tick' });
      }
    } else {
      // gentle hover near Gebäude 125 on the service apron (no cross-field drift)
      const wob = Math.sin(Date.now()/3500) * 0.000012;
      lat = DEMO_ORIGIN.lat + wob;
      lng = DEMO_ORIGIN.lng + wob*0.6;
      hdg = DEMO_ORIGIN.heading;
      setCurrentPosition(lat, lng, { accuracy:8, heading:hdg, speed: 0, source:'demo-tick' });
    }
    updateGPSStatus('demo');
  }, 1000);
}
...
function registerSW(){
  if('serviceWorker' in navigator && location.protocol !== 'file:'){
    navigator.serviceWorker.register('sw.js?v=22').catch(err=>console.warn('SW',err));
  }
}
function bindEvents(){
